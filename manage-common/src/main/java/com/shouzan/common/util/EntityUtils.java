package com.shouzan.common.util;
...
public class EntityUtils {
	...
	public static <T> void setCreateInfo(T entity){
		...
		if(request != null && request.getHeader("UserID") != null) {
			id = new String(Base64Utils.decodeFromString(request.getHeader("UserID")));
		}
		
		Date date = new Date();
		// 默认属性
		String[] fields = {"creatorId","createTime","checkerId","checkTime"};
		Object[] values = {id, date, id, date};
		// 填充默认属性值
	...
	public static <T> void setUpdatedInfo(T entity){
		...
		Date date = new Date();
		// 默认属性
		String[] fields = {"lastEdittime","lastEditorid"};
		Object[] values = {date, id};
		// 默认属性
		// 填充默认属性值
		setDefaultValues(entity, fields, values);
	}
